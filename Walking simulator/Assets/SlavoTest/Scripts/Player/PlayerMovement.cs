...
namespace SlavoTest.Player
{
    public class PlayerMovement : MonoBehaviour
    {
        ...
        [Header("Jump")] [SerializeField] private float _jumpForce = 500.0f;
        ...
        {
            _isGrounded = (Mathf.Abs(_playerRigidbody.velocity.y - _previousYVelocity) < .1f) &&
                          (Physics.OverlapSphere(_groundChecker.position, _groundCheckerDist).Length >
                           1); // > 1 because it also counts the player
        ...
        private void HandleMovement()
        {
            ...
            _inputForce = (transform.forward * _verticalInput + transform.right * _horizontalInput).normalized *
                          (Input.GetKey(KeyCode.LeftShift) ? _runSpeed : _walkSpeed);
            ...
            if (_isGrounded)
            {
                ...
                _playerRigidbody.velocity =
                    Vector3.Lerp(_playerRigidbody.velocity, _inputForce, 10 * Time.fixedDeltaTime);
            }
            else
            {
                _playerRigidbody.velocity = ClampSqrMag(_playerRigidbody.velocity + _inputForce * Time.fixedDeltaTime,
                    _playerRigidbody.velocity.sqrMagnitude);
            }
        }
